        ...
        <div className="flex items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6 flex-shrink-0">
            ...
            <Link to="/" className="flex-shrink-0">
              <div
                className={`relative ${bgColor} backdrop-blur-xl ${textColor} px-4 py-2 rounded-full ${borderColor} border hover:bg-white/20 transition-all shadow-lg shadow-black/5`}
              >
            ...
            <div
              className={`hidden lg:flex items-center gap-1.5 px-3 py-2 ${bgColor} backdrop-blur-xl ${borderColor} border rounded-full shadow-lg shadow-black/5 relative`}
            >
              <div className="absolute inset-0 bg-gradient-to-br from-white/40 via-white/20 to-transparent rounded-full pointer-events-none"></div>
              <div className="w-1.5 h-1.5 bg-lime-500 rounded-full animate-pulse relative z-10"></div>
              <span className={`text-xs ${textColor} relative z-10`}>
          ...
          {/* Center - Desktop Menu */}
          <div
            className={`hidden md:flex items-center gap-0.5 ${bgColor} backdrop-blur-xl ${borderColor} border rounded-full px-1.5 py-1.5 shadow-lg shadow-black/5 relative`}
          >
            <div className="absolute inset-0 bg-gradient-to-br from-white/40 via-white/20 to-transparent rounded-full pointer-events-none"></div>
            {navLinks.map((link) => {
          ...
          <div className="hidden lg:flex items-center flex-shrink-0">
            <Link
              to="/contact"
              className={`group relative ${bgColor} backdrop-blur-xl ${textColor} h-[42px] px-6 rounded-full ${borderColor} border shadow-lg shadow-black/5 overflow-hidden transition-all hover:scale-105 flex items-center justify-center min-w-[180px]`}
            >
              <div className="absolute inset-0 bg-gradient-to-br from-white/40 via-white/20 to-transparent rounded-full pointer-events-none"></div>
              ...
              <div className="relative w-full h-full overflow-hidden">
                {/* First text state */}
                <div className="absolute inset-0 flex items-center justify-center gap-2 text-xs font-medium uppercase tracking-wider transition-all duration-500 ease-in-out group-hover:opacity-0 group-hover:-translate-y-4">
                  Get in Touch
                  <ArrowRight size={14} className="animate-icon-pulse" />
                </div>

                {/* Second text state */}
                <div className="absolute inset-0 flex items-center justify-center gap-2 text-xs font-medium uppercase tracking-wider transition-all duration-500 ease-in-out opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0">
                  <ArrowRight size={14} className="animate-icon-pulse" />
                  Contact Me
                </div>
              </div>
